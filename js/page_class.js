function HC_CLS() {
    ...
    var a = document.getElementById("display");//target
    ...
    window.team[0] = { tid: 0, t_name: "", t_player: "", t_sub: "", t_type: null };

    //创建表格框架
    for (var i = window.cls[0]["round"]; i--;) {
        var new_table = ce(["table",
            "id", "t" + (i + 1),
            "class", "bordered"
        ]);
        var new_tr = document.createElement("tr");
        var new_th = document.createElement("th");
        new_th.setAttribute("colspan", is4 ? 5 : 4);

        var new_a = document.createElement("a");
        new_a.setAttribute("class", "radius_a");
        new_a.setAttribute("name", "_class" + (i + 1));
        new_a.innerText =
            window.admin.c_name +
            "　第" +
            (i + 1) +
    ...
    for (var i = 0; i < window.cls.length; i++) {
        var row = window.cls[i];
        
        var team1 = window.team[row["tid1"]];
        var team2 = window.team[row["tid2"]];
        var team3 = window.team[row["tid3"]];
        var team4 = window.team[row["tid4"]];

        var new_tr = document.createElement("tr");
        var new_td = document.createElement("td");
        new_td.setAttribute("colspan", is4 ? 5 : 4);
